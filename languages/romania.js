		...
		this.language = {
			...
			COMMANDS_SERVER: {
				...
				statsValue: (createdAt, owner, channels, roles, members, bots) =>
					[`Creat [**${createdAt}**]`, `👑 [**${owner}**]`, `Canale [**${channels}**]`, `Roluri [**${roles}**]`, `👥 [**${members}**]`, `🤖 [**${bots}**]`].join('\n'),
				settingsName: 'Setarile specifice G4M3R ',
				settingsValue: settings =>
					[
						`Limba: ${settings.language}`,
						`Roluri de moderare: ${settings.settings.moderation.modRoles.map(roleID => `<@&${roleID}>`).join(' ')}`,
						`Admini: ${settings.settings.admins.map(userID => `<@${userID}>`).join(' ')}`,
						`Rol primit automat: ${settings.settings.autoAssignRoles.mainRole ? `<@&${settings.settings.autoAssignRoles.mainRole}>` : ''}`,
						...
						`Rol de verificare: ${settings.settings.verify.role ? `<@&${settings.settings.verify.role}>` : 'None'}`,
						`Categoria Mail: ${settings.settings.mail.mailCategory ? settings.settings.mail.mailCategory : 'None'}`,
						`Categoria de verificare: ${settings.settings.verify.category}`,
						`Canale de verificare deschise: ${settings.welcomeChannels.length}`,
						// `Verification Main Channel: ${settings.channels.verifyMainChannelID}`,
						`Mesaje de AFK inregistrate: ${settings.settings.allowAfkResponses}`,
						`Feedback Feature: **Idee:** ${settings.settings.feedback.idea.status} | **Buguri**: ${settings.settings.feedback.bug.status}`
					].join('\n')
			},
			BAD_EMBED: 'Acesta nu este un **embed valid**. Cel mai bine folositi acest embed builder <https://embedbuilder.nadekobot.me/>!',
			CS: 'CS',
			DAYS: 'Zile',
			DEFEAT: 'Infrant',
			DURATION: 'Durata',
			...
			FEEDBACK_EDIT_NOT_FOUND: id => `Nu am putut gasi mesajul de feedback cu ID-ul: ${id}`,
			FEEDBACK_EDIT_NOT_AUTHOR: 'Nu poti edita aces mesaj de feedback deoarece tu nu esti autorul.',
			FEEDBACK_EDIT_SUCCESS: 'Ti-ai editat cu succes mesajul de feedback.',
			FEEDBACK_ORIGINAL_NOT_FOUND: 'Mesajul original de feedback nu a putut fi gasit ,deci nu il pot edita.',
			FEEDBACK_FOOTER: (prefix, action, id) => `Pentru a trimite un feedback: ${prefix}fb ${action} | Pentru a edita feedback-ul: ${prefix}fb edit ${id}`,
			FEEDBACK_FROM: (type, authorName) => `${klasaUtil.toTitleCase(type)} De la ${authorName}`,
			...
			FEEDBACK_SENT: 'Feedback-ul tau a fost trimis, mersi!',
			...
			MENU_CANCEL_OPTIONS: ['i', 'iesi'],
			MINUTES: 'Minute',
			NOT_A_MOD: 'Nu esti moderator al acestui server. Te rog contacteaza un moderator sau admin.',
			...
			SETTINGS_LEVELS_MENU_LIST_DATA: xp => [
				'Creaza un nivel',
				'Sterge un nivel',
				'Editeaza numele unui nivel',
				'Editeaza XP-ul necesar unui nivel',
				'Editeaza recompensa unui nivel',
				'Editeaza XP-ul primit per comanda',
				`XP primit per mesaj [${xp.pointsPerMessage}]`,
				`Server: Notificari **in canalul curent** [**${xp.notification.server.channel ? 'On' : 'Off'}**]`,
